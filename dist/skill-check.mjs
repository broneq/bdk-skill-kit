...
// src/rules/cli-front.ts
var escape = (text) => text.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
var FLAG = /(?<![\w-])--[a-z][a-z0-9-]*/g;
var FREE_FLAGS = /* @__PURE__ */ new Set(["--help", "--json"]);
var cliFront = defineRule({
  id: "cli-front",
  kinds: ["skills"],
  ...
  check(doc, ctx) {
    ...
    const command = typeof metadata === "object" && metadata !== null ? metadata["fronts-cli"] : void 0;
...
var ABSOLUTE = /(?<![\w$}.:/~-])(?:\/(?:Users|home|root|opt|var|tmp|etc|private)\/\S*|~\/\S*|[A-Za-z]:\\\S*)/g;
var absolutePaths = defineRule({
  id: "absolute-paths",
  kinds: ["skills", "agents"],
  defaultSeverity: "error",
  check(doc, ctx) {
...
var fields = defineRule({
  ...
  check(doc, ctx) {
    ...
    for (const key2 of Object.keys(fm)) {
      if (allowed.includes(key2)) continue;
      const line = lineOf(doc, key2);
      let message;
      if (doc.kind === "agents" && PLUGIN_IGNORED_AGENT_FIELDS.includes(key2)) {
        message = `\`${key2}\` is ignored for plugin agents (${PLUGIN_IGNORED_AGENT_FIELDS.join(", ")}); move the agent to .claude/agents/ or drop the field`;
      } else if (doc.target.profile === "portable") {
        message = `\`${key2}\` is not a field of the portable profile (Agent Skills standard: ${PORTABLE_FIELDS.join(", ")})`;
      } else {
        message = `\`${key2}\` is not a field of the claude-code ${doc.kind} profile`;
      }
      ctx.report({ line, message, match: key2 });
    }
  }
});
...
var fieldValues = defineRule({
  ...
  check(doc, ctx) {
    ...
    if (!fm) return;
    const allowed = allowedFields(doc.kind, doc.target.profile);
    const has = (key2) => key2 in fm && allowed.includes(key2);
    const fail = (key2, message) => {
      ctx.report({ line: lineOf(doc, key2), message, match: key2 });
    };
    const oneOf = (key2, values, message) => {
      if (has(key2) && !values.includes(fm[key2])) fail(key2, message);
    };
    ...
    for (const key2 of [
      ...
      if (has(key2) && key2 !== "name" && key2 !== "description" && typeof fm[key2] !== "string") {
        fail(key2, `\`${key2}\` must be a string`);
      }
    }
    for (const key2 of [
      "disable-model-invocation",
    ...
    if (has("metadata")) {
      const m = fm.metadata;
      const ok = typeof m === "object" && m !== null && !Array.isArray(m) && Object.values(m).every((v) => typeof v === "string");
    ...
    for (const key2 of ["tools", "disallowedTools"]) {
      ...
      for (const entry of entries) {
        if (!TOOL.test(entry.trim())) {
          fail(
            key2,
            `\`${key2}\` entry \`${entry.trim()}\` is not a tool name, \`Name(...)\` or \`mcp__...\``
          );
        }
      }
    }
    if (has("maxTurns") && !(Number.isInteger(fm.maxTurns) && fm.maxTurns > 0)) {
  ...
  check(doc, ctx) {
    ...
    if (fm["disable-model-invocation"] === true && fm["user-invocable"] === false) {
      ctx.report({
        ...
        message: "`disable-model-invocation: true` with `user-invocable: false` leaves no way to run the skill"
      });
    }
...
function readFrontmatter(lines) {
  ...
  const doc = (0, import_yaml.parseDocument)(source, { lineCounter: counter });
  ...
  if (doc.contents === null) return { map: {}, error: void 0, keyLines: {}, bodyStart };
  if (!(0, import_yaml.isMap)(doc.contents)) return failed("the frontmatter is not a YAML map", bodyStart);
  const keyLines = {};
  for (const pair of doc.contents.items) {
    if ((0, import_yaml.isScalar)(pair.key)) {
      keyLines[String(pair.key.value)] = counter.linePos(pair.key.range[0]).line + 1;
    }
  }
  return { map: doc.toJS(), error: void 0, keyLines, bodyStart };
}
function failed(error, bodyStart) {
  return { map: void 0, error, keyLines: {}, bodyStart };
}
function fenceMask(lines, from) {
  ...
  for (let i = from - 1; i < lines.length; i++) {
    ...
    if (open === void 0) {
      ...
      mask[i] = true;
      if (marker?.startsWith(open[0] ?? "") && marker.length >= open.length) open = void 0;
    }
  }
  return mask;
}
...
var LINK = /\[[^\]]*\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g;
var TICKED = /`([^`\s]+)`/g;
var SKILL_DIR = /\$\{CLAUDE_SKILL_DIR\}\/([^\s`'")]+)/g;
var CONVENTIONAL_DIRS = ["references", "scripts", "assets", "examples", "templates"];
function references(lines, inFence, topLevel) {
  const out = [];
  lines.forEach((text, i) => {
    ...
    for (const m of text.matchAll(TICKED)) {
      ...
      if (target.includes("/") && isLocal(target) && (CONVENTIONAL_DIRS.includes(first) || topLevel.has(first))) {
...
var clean = (target) => target.replace(/#.*$/, "").replace(/^\.\//, "").replace(/[.,;:]+$/, "");
var isLocal = (t) => t !== "" && !/^(?:[a-z][a-z0-9+.-]*:|#|\/|~|\$|\.\.\/)/i.test(t) && !t.includes("${");
function topLevelOf(doc) {
...
var isText = (file) => /\.(md|markdown|txt)$/i.test(file);
var referencesRule = defineRule({
  id: "references",
  kinds: ["skills"],
  defaultSeverity: "error",
  check(doc, ctx) {
...
var CONFIG_NAMES = ["skill-check.config.ts", "skill-check.config.mjs", "skill-check.config.js"];
async function loadConfig(cwd, explicit) {
  const file = explicit ? resolve(cwd, explicit) : CONFIG_NAMES.map((n) => resolve(cwd, n)).find(existsSync);
  ...
  return {
    ...
    rules,
    settings,
    baseline: config.baseline === void 0 ? void 0 : resolve(root, config.baseline)
  };
}
function loadTarget(value, index, root, rules, settings) {
  const target = typeof value === "object" && value !== null ? value : {};
  const dirs = Array.isArray(target.dirs) ? target.dirs : [];
  const name = typeof target.name === "string" ? target.name : typeof dirs[0] === "string" ? dirs[0] : `#${index + 1}`;
  const fail = (message) => new ConfigError(`target \`${name}\`: ${message}`);
...
function settingOf(rule, settings) {
  const own = settings[rule.id];
  const parsed = own === void 0 ? { severity: rule.defaultSeverity, options: {} } : parseSetting(rule.id, own);
  return {
      ...
      if (target.kind === "agents") {
        for (const entry of sorted(base)) {
          if (entry.isFile() && entry.name.endsWith(".md")) {
            const file = join2(base, entry.name);
...
function gitFiles(dir) {
  ...
  if (run2.error || run2.status !== 0) return null;
  return [...new Set(run2.stdout.split("\0"))].filter((f) => f && !f.split("/").some((part) => part.startsWith(".")) && isFile(join2(dir, f))).sort((a, b) => a.localeCompare(b));
}
...
function walk(dir, prefix = "") {
  ...
  for (const entry of sorted(dir)) {
    ...
    if (entry.isDirectory()) out.push(...walk(join2(dir, entry.name), rel));
...
// src/runner.ts
function runChecks(config, options) {
  const { docs, strays } = discover(config.root, config.targets);
  const findings = [];
  const narrow = options.paths.map(
    (p) => relative2(config.root, resolve2(options.cwd, p)).split(sep2).join("/")
  );
  const inScope = (file) => narrow.length === 0 || narrow.some((p) => file === p || file.startsWith(`${p}/`));
  const selected = (doc) => inScope(doc.path);
...
function context(config, rule, severity, options, file, sink) {
  return {
    options,
    root: config.root,
    report(report) {
...
function fingerprint(rule, file, text) {
...
function enabledRules(config) {
  const all = [config.settings, ...config.targets.map((t) => t.settings)];
  return [...config.rules.values()].filter((rule) => all.some((settings) => settingOf(rule, settings).severity !== "off")).map((rule) => rule.id).sort();
}

// src/baseline.ts
var key = (e) => `${e.rule}\0${e.file}\0${e.fingerprint}`;
var isEntry = (value) => {
  if (typeof value !== "object" || value === null) return false;
  const { rule, file, fingerprint: fingerprint2 } = value;
  return typeof rule === "string" && typeof file === "string" && typeof fingerprint2 === "string";
};
function readBaseline(path, shown) {
  if (!existsSync2(path)) {
  ...
  return parsed.map(({ rule, file, fingerprint: fingerprint2 }) => ({ rule, file, fingerprint: fingerprint2 }));
}
function writeBaseline(path, entries) {
  const sorted2 = [...entries].sort((a, b) => key(a).localeCompare(key(b)));
  writeFileSync(path, `${JSON.stringify(sorted2, null, 2)}
`);
}
function entriesOf(findings) {
  return findings.map(({ rule, file, fingerprint: fingerprint2 }) => ({ rule, file, fingerprint: fingerprint2 }));
}
function applyBaseline(findings, entries, inScope) {
...
// src/report.ts
function formatHuman(outcome, github) {
  const { errors, warnings } = count(outcome.findings);
  const plural = (n, word) => `${n} ${word}${n === 1 ? "" : "s"}`;
  const files = plural(outcome.files, "file");
  if (outcome.findings.length === 0) {
    const suppressed2 = outcome.suppressed ? ` (${plural(outcome.suppressed, "baselined finding")})` : "";
    return `No findings in ${files}${suppressed2}.
`;
  }
  ...
  const suppressed = outcome.suppressed ? `, ${plural(outcome.suppressed, "baselined finding")} suppressed` : "";
  return `${lines.join("\n")}

${plural(errors, "error")}, ${plural(warnings, "warning")} in ${files}${suppressed}.
`;
}
function formatJson(outcome) {
  const { errors, warnings } = count(outcome.findings);
...
var USAGE = `Usage: skill-check [paths...] [options]
...
async function main(argv, io) {
  try {
    ...
    if (error instanceof ConfigError || isArgError(error)) {
      io.stderr(`skill-check: ${error.message}
`);
      return 2;
    }
    throw error;
...
`);
  ...
  if (values["list-rules"]) {
    io.stdout(
      values.json ? `${JSON.stringify(result.enabledRules)}
` : `${result.enabledRules.join("\n")}
`
    );
    return 0;
  }
  const baseline = values.baseline === void 0 ? config.baseline : resolve3(io.cwd, values.baseline);
  const shown = baseline === void 0 ? "" : relative3(config.root, baseline);
...
`);
  ...
  let entries = baseline === void 0 ? [] : readBaseline(baseline, shown);
  let applied = applyBaseline(result.findings, entries, result.inScope);
  if (prune && baseline !== void 0) {
