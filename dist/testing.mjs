...
import { dirname as dirname2, isAbsolute, join as join3, normalize, sep as sep3 } from "node:path";
...
// src/rules/frontmatter.ts
var lineOf = (doc, key) => doc.keyLines[key] ?? 1;
...
var fieldValues = defineRule({
  ...
  check(doc, ctx) {
    ...
    const oneOf = (key, values, message) => {
      if (has(key) && !values.includes(fm[key])) fail(key, message);
    };
    ...
    for (const key of ["allowed-tools", "disallowed-tools", "arguments", "paths", "skills"]) {
      if (has(key) && !isStringList2(fm[key]))
...
// src/testing.ts
async function checkRule(rule, test) {
  for (const path of Object.keys(test.files)) {
    if (isAbsolute(path) || normalize(path).split(sep3)[0] === "..") {
  ...
  const settings = {
    [rule.id]: test.options === void 0 ? severity : [severity, test.options]
  };
  const rules = /* @__PURE__ */ new Map([[rule.id, rule]]);
  const kind = test.kind ?? "skills";
  const root = await mkdtemp(join3(tmpdir(), "skill-check-rule-"));
  try {
